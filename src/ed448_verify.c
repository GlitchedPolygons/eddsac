...
int main(const int argc, const char* argv[])
{
    ...
    {
        fprintf(stdout, "ed448_verify:  Verify an Ed448 signature using a specific public key. Call this program using exactly 3 arguments;  the FIRST one being the PUBLIC KEY (hex-string), the SECOND one being the SIGNATURE to verify (also a hex-string) and the THIRD one the actual STRING TO VERIFY the signature against.\n");
        return 0;
    }

    if (argc != 4)
    {
        fprintf(stderr, "ed448_verify: wrong argument count. Check out \"ed448_verify --help\" for more details about how to use this!\n");
        return -1;
    }

    int r = -1;

    uint8_t public_key[DECAF_EDDSA_448_PUBLIC_BYTES + 1];
    ...
    if (public_key_hexstr_length != DECAF_EDDSA_448_PUBLIC_BYTES * 2)
    {
        fprintf(stderr, "ed448_verify failed: Invalid public key format/length!\n");
        r = -2;
        goto exit;
    }

    if (signature_hexstr_length != DECAF_EDDSA_448_SIGNATURE_BYTES * 2)
    {
        fprintf(stderr, "ed448_verify failed: Invalid signature format/length!\n");
        r = -2;
        goto exit;
    }
    ...
    {
        fprintf(stderr, "ed448_verify failed: The signature is invalid.\n");
        r = -3;
        goto exit;
    }
    
    fprintf(stdout, "ed448_verify: Success! The signature is valid.\n");
    r = 0;
