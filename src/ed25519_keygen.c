...
{
    uint8_t privatekey[DECAF_EDDSA_25519_PRIVATE_BYTES];
    uint8_t publickey[DECAF_EDDSA_25519_PUBLIC_BYTES];
    ...
    decaf_spongerng_next(  
      ...
      DECAF_EDDSA_25519_PRIVATE_BYTES /* Number of bytes to output. */
    );

    memcpy(privatekey, csprng_buffer, DECAF_EDDSA_25519_PRIVATE_BYTES);

    decaf_ed25519_derive_public_key(publickey, privatekey); 
